...
class SSDownlink:
    def __init__(self, angel1, angel2, D, f, T, bandwidth, bandwidth2, data_rate, pt_list,
        ...
        # 单个干扰卫星参数
        self.tx_power = tx_power  # 通信链路 发射端天线功率，单位：瓦特 低轨道卫星通信系统，发射功率一般在数瓦到几十瓦之间
    ...
    def calculate_I_N(self):
        ...
        for i in range(num_terms):
            ...
            term = tr_power * gt_list * rx_antenna_gain * (
                    wavelength / (4 * math.pi * 1000 * self.distance_to_satellite_list[i])) ** 2
            term1 += term

        I = 10 * math.log10(term1)
    ...
    def calculate_C_N(self):
        ...
        tx_antenna_gain = NGSO_SAT_1528_1(self.D, self.f, 0)
        rx_antenna_gain = self.rx_antenna_gain  # 设置固定值
        wavelength_db = 20 * math.log10(self.wavelength_list[0])  # 下行链路工作载波的波长
        C = tx_antenna_gain + rx_antenna_gain + tx_power_db + wavelength_db - distance_to_satellite - loss_pg
        N = 10 * math.log10(self.k * self.T * self.bandwidth2)
        C_N = C - N
        return C_N, C
